...
def req_coverage(dsg: DesignState, system_type: str = "water") -> float:
    ...
    hits = dict.fromkeys(req_patterns, False)
...
def embodiment_ratio(dsg: DesignState) -> float:
    ok = [n for n in dsg.nodes.values() if n.embodiment]  # and n.embodiment.principle != "undefined"]
...
def _script_ok(p: Path) -> bool:
    try:
        ...
        r = subprocess.run([sys.executable, str(p), "--help"], capture_output=True, timeout=PY_TIMEOUT)
...
def _compute_wall_time(snaps: list[Path]) -> float:
    t0 = datetime.fromtimestamp(snaps[0].stat().st_mtime, tz=UTC)
    t1 = datetime.fromtimestamp(snaps[-1].stat().st_mtime, tz=UTC)
    ...
    with log_file.open() as f:
        for line in f:
            if line.strip():
                ...
                run_data_map[run_id] = {"success": success, "wall_time": wall_time}
...
def generate_report(df: pd.DataFrame, output_dir: Path, batch_id: str):
    ...
    try:
        ...
        _fig, axes = plt.subplots(len(metrics), 1, figsize=(10, 4 * len(metrics)))
...
def format_value_with_bold(
    value: str, metric: str, best_values: dict[str, Any], stats: pd.DataFrame, llm_type: str, temp: float, workflow: str
) -> str:
...
def generate_latex_table(df: pd.DataFrame, output_path: Path, batch_id: str):
    ...
    try:
        ...
        # LLM name mapping
        llm_names = {"reasoning": "DeepSeek R1 70B", "non_reasoning": "Llama 3.3 70B"}
        ...
        with Path(output_path).open("w") as f:
            ...
            f.write(r"  \centering" + "\n")
            f.write(
                r"  \caption{Overall performance (mean\,$\pm$\,std over 10 runs) of each LLM under the multi-agent system (MAS) and two-agent system (2AS) across temperature settings. Best values in \textbf{bold}.}"
                + "\n"
            )
            ...
            f.write(r"    \toprule" + "\n")
            f.write(
                r"    \textbf{LLM} & \textbf{System} & \textbf{Temp} & \textbf{M1 (\%)$\uparrow$} & \textbf{M2 (\%)$\uparrow$} & \textbf{M3 (\%)$\uparrow$} & \textbf{M4 (\%)$\uparrow$} & \textbf{M5 (\#)$\uparrow$} & \textbf{M6 (s)$\downarrow$} & \textbf{M7 (\# N)$\uparrow$} \\"
                + "\n"
            )
            f.write(r"    \midrule" + "\n")
                ...
                for j, workflow in enumerate(workflows):
                    ...
                    for k, temp in enumerate(temperatures):
                        ...
                        for metric in ["M1", "M2", "M3", "M4", "M5", "M6", "M7"]:
                            formatted_val = format_value_with_bold(
                                "", metric, best_values, stats, llm_type, temp, workflow
                            )
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Evaluate batch of DSG runs and generate report.")
    ap.add_argument(
        "--batch-id", required=True, help="Timestamp of the batch folder to evaluate (e.g. 20250612_153045)"
    )
    ap.add_argument("--base-dir", default="runs", help="Base directory containing batch folders (default: runs/)")
    ap.add_argument("--output-dir", default="experiment_results", help="Directory to save aggregated results and plots")
    ap.add_argument(
        "--system-type",
        choices=["water", "uam"],
        default=None,
