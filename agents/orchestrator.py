...
def orchestrator_node(state: State) -> Command[Literal["worker"]]:
    ...
    """
    print(f"🧠 Orchestrator invoked by {state.current_requesting_agent}")

    if not state.orchestrator_orders:
        print("⚠️ No orchestrator orders provided. Returning to specialized agent.")
        return Command(
            update={"messages": [AIMessage(content="⚠️ No orders provided to the orchestrator.")]},
    ...
    messages = [
        ...
        HumanMessage(
            state.orchestrator_orders[-1]
        ),  # TODO: check it does not bug with removal of [] in generation agent
        HumanMessage(
    ...
    print("🟢 Invoking CIA model to generate tasks...")
    cia_output = cia_model.invoke(messages)

    display(Markdown(f"**CIA Response:** {cia_output.response}"))

    if not cia_output.research_tasks:
        print("⚠️ No tasks generated. Returning to specialized agent.")
        return Command(
            update={
                "messages": [
                    AIMessage(content=cia_output.response),
                    AIMessage(content="⚠️ No tasks were generated by the orchestrator."),
                ],
            },
            goto=state.current_requesting_agent,
        )
    ...
    print(f"🟢 Dispatching {task_count} tasks to Worker Agents.")

    return Command(
        update={"messages": [AIMessage(content=cia_output.response)], "current_tasks_count": task_count},
