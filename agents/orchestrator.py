...
def orchestrator_node(state: State) -> Command[Literal["worker"]]:
    ...
    if not cia_output.research_tasks:
        ...
        return Command(
            update={
                "messages": [AIMessage(content=cia_output.response), AIMessage(content="⚠️ No tasks were generated by the orchestrator.")],
            },
            goto=state.current_requesting_agent
        )
    ...
    return Command(
        update={
            "messages": [AIMessage(content=cia_output.response)],
